...
public class TmpLinkCommand {
  ...
  public TmpLinkCommand(TmpLinkPlugin plugin) {
    this.plugin = plugin;
  }

  public void execute(CommandSender sender, String[] args) {
    if (!sender.hasPermission("tmplink.admin")) {
      String version = this.plugin.getPlatform().getPluginVersion();
      sender.sendMessage("&9TmpLink v" + version + "&7. Website: &9https://temporal.fr");
    ...
    if (args[0].equalsIgnoreCase("setup")) {
      if (args.length < 3) {
        sender.sendMessage("&cUsage: /tmplink setup <url> <key>");
        return;
      }
    ...
    if (args[0].equalsIgnoreCase("port")) {
      if (args.length < 2) {
        sender.sendMessage("&cUsage: /tmplink port <port>");
        return;
      }
  ...
  public void editMoney(CommandSender sender, String[] args) throws NumberFormatException {
    if (args.length < 4 || !MONEY_ACTIONS.contains(args[1].toLowerCase())) {
      sender.sendMessage("&cUsage: /tmplink money <add|remove|set> <player> <amount>");
      return;
    }
    ...
    if (!user.isPresent()) {
      sender.sendMessage("&cUnable to find player '" + args[2] + "', please try again in few seconds or use '/tmplink fetch'.");
      return;
    }
  ...
  public String getUsage() {
    return "Usage: /tmplink [" + String.join("|", COMPLETIONS) + "]";
  }

  private void sendUsage(CommandSender sender) {
    String version = this.plugin.getPlatform().getPluginVersion();
    sender.sendMessage("&9TmpLink v" + version + "&7. Website: &9https://temporal.fr");
    sender.sendMessage("&8- /tmplink setup <url> <key>");
    sender.sendMessage("&8- /tmplink port <port>");
    sender.sendMessage("&8- /tmplink status");
  }
