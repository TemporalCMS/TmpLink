package fr.temporal.tmplink.common.http.client;

import com.google.gson.JsonObject;
import fr.temporal.tmplink.common.TmpLinkPlugin;
import fr.temporal.tmplink.common.data.ServerData;
import fr.temporal.tmplink.common.data.UserInfo;
import fr.temporal.tmplink.common.data.WebsiteResponse;
import fr.temporal.tmplink.common.users.EditMoneyResult;
...
public class HttpClient {
  ...
  public HttpClient(TmpLinkPlugin plugin) {
    this.plugin = plugin;
  }

  public CompletableFuture<Void> verifyStatus() {
    return request(RequestMethod.GET, "/tmplink", null);
  }
  ...
  public CompletableFuture<Void> registerUser(String name, String email, UUID uuid, String password, InetAddress address) {
    ...
    return request(RequestMethod.POST, "/tmplink/register", params);
  }
  ...
  public CompletableFuture<Void> updateEmail(UUID uuid, String email) {
    ...
    return request(RequestMethod.POST, "/tmplink/email", params);
  }

  public CompletableFuture<EditMoneyResult> editMoney(UserInfo user, String action, double amount) {
    String endpoint = "/tmplink/user/" + user.getId() + "/money/" + action;
    JsonObject params = new JsonObject();
  ...
  public CompletableFuture<WebsiteResponse> postData(ServerData data) {
    return request(RequestMethod.POST, "/tmplink", data, WebsiteResponse.class);
  }
  ...
  public <T> CompletableFuture<T> request(RequestMethod method, String endpoint, Object params, Class<T> clazz) {
    String body = TmpLinkPlugin.getGson().toJson(params);
  ...
  private <T> T rawRequest(RequestMethod method, String endpoint, String body, Class<T> clazz)
    ...
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getInputStream()))) {
      T response = TmpLinkPlugin.getGson().fromJson(reader, clazz);
  ...
  private HttpURLConnection prepareConnection(RequestMethod method, String endpoint) throws IOException {
    ...
    conn.addRequestProperty("Temporal-Link-Token", token);
    conn.addRequestProperty("Content-Type", "application/json; charset=utf-8");
    conn.addRequestProperty("User-Agent", "TmpLink java v" + version);
