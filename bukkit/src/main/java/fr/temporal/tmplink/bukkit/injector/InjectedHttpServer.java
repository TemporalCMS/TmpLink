...
public class InjectedHttpServer implements HttpServer {
  ...
  public InjectedHttpServer(TmpLinkBukkitPlugin plugin) {
    this.plugin = plugin;
  }
  ...
  public void start() {
    ...
    if (!HttpDecoder.class.getSuperclass().getName().startsWith("io.")) {
      plugin.getLoggerAdapter().error("Injecting HTTP server on server channel is not supported with TmpLink legacy.");
      return;
    }

    if (!Bukkit.getServer().getClass().getSimpleName().equals("CraftServer")) {
      plugin.getLoggerAdapter().error("Injecting HTTP server on server channel is only supported on CraftBukkit based servers. You can use an other port for TmpLink");
      return;
    }
    ...
    try {
      ...
      plugin.getLoggerAdapter().error("Unable to inject HTTP server. Try using a different port for TmpLink", e);
    }
  }
